...
class PlotMarginalModes(object):
    ...
    def plot_conditional(self, dim1, dim2 = None,with_ellipses = True, with_points = True,only_interpolate = False, use_log_values = False,grid_points = 40, marginalization_type='sum'):
        """
        ...
        if dim2 is not None:
            ...
            plt.imshow(grid_z.transpose(), origin='lower', aspect='auto',
                cmap=cm.PuBu, alpha = 0.8, extent=(min1,max1,min2,max2))
			#plt.colorbar()
        else:
			#plt.xlim(min1, max1)
            plt.plot(grid_x, grid_z[:,0], '-', color='#58ACFA', drawstyle='steps')
            plt.grid(False)
		# add contours
        if use_log_values:
