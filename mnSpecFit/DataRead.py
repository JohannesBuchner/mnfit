...
class DataRead:
    ...
    def ReadData(self,chans=[0,140],bkgIntervals=[[-20,-.1],[50.,300.]]):
        '''
    ...
    def CreateCounts(self,ext="",start=0,stop=10):
        ...
        for i in range(len(bins)-1):
            ...
            if lob>=start and hib<=stop:
                ...
                tab.meta={"tBin":[lob,hib],"INST":self.instrument,"DET":self.det,"RSP":self.rsp}
                directory = "bin%d"%j

                make_sure_path_exists(ext+directory)
                
                
                tab.write(ext+directory+"/"+self.instrument+"_"+self.det+".fits",format="fits",overwrite=True)
                j+=1
